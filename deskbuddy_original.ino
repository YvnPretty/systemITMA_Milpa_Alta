...
void drawFilledHeart(int cx, int cy, int size, uint16_t color) {
  int r = size / 2;
  if (r < 2) r = 2;
  display.fillCircle(cx - r / 2, cy - r / 3, r / 2 + 1, color);
  display.fillCircle(cx + r / 2, cy - r / 3, r / 2 + 1, color);
  display.fillTriangle(cx - r - 1, cy - r / 4, cx + r + 1, cy - r / 4, cx, cy + r + 1, color);
}
...
void drawLoveEye(Eye& e, bool isLeft) {
  ...
  drawFilledHeart(cx, cy, outerSize, SH110X_WHITE);

  if (innerSize >= 3) {
    drawFilledHeart(cx, cy, innerSize, SH110X_BLACK);
    display.fillCircle(cx + innerSize / 3, cy - innerSize / 3, 1, SH110X_WHITE);
  }

  int sparkOffset = (int)(sin(millis() / 180.0f + (isLeft ? 0.0f : 3.14f)) * 2.0f);
  if (isLeft) {
    display.drawPixel(cx - outerSize - 4, cy - 4 + sparkOffset, SH110X_WHITE);
    display.drawPixel(cx - outerSize - 3, cy - 3 + sparkOffset, SH110X_WHITE);
    display.drawPixel(cx + outerSize + 3, cy - 6 - sparkOffset, SH110X_WHITE);
  } else {
    display.drawPixel(cx + outerSize + 4, cy - 4 + sparkOffset, SH110X_WHITE);
    display.drawPixel(cx + outerSize + 3, cy - 3 + sparkOffset, SH110X_WHITE);
    display.drawPixel(cx - outerSize - 3, cy - 6 - sparkOffset, SH110X_WHITE);
...
void drawHappyEye(Eye& e, bool isLeft) {
  ...
  display.fillCircle(cx, cy, radius, SH110X_WHITE);
  display.fillCircle(cx, cy + 3, radius - 3, SH110X_BLACK);
  display.fillRect(cx - radius - 3, cy + 1, (radius + 3) * 2, radius + 6, SH110X_BLACK);

  display.drawPixel(cx - radius, cy, SH110X_WHITE);
  display.drawPixel(cx - radius + 1, cy - 1, SH110X_WHITE);
  display.drawPixel(cx + radius, cy, SH110X_WHITE);
  display.drawPixel(cx + radius - 1, cy - 1, SH110X_WHITE);

  if (isLeft) {
    display.drawFastHLine(cx - radius + 1, cy + 6, 4, SH110X_WHITE);
    display.drawFastHLine(cx - radius + 2, cy + 8, 4, SH110X_WHITE);
  } else {
    display.drawFastHLine(cx + radius - 5, cy + 6, 4, SH110X_WHITE);
    display.drawFastHLine(cx + radius - 6, cy + 8, 4, SH110X_WHITE);
  }
}
